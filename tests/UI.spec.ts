...
test.describe('UI Tests', () => {
	test('Home page', async ({ page }) => {
		...
		await test.step('Check top bar links exist', async () => {
			...
			await expect(page.getByRole('link', { name: 'Reiverr', exact: true })).toHaveAttribute(
				'href',
				'/'
			);
	...
	test('Settings', async ({ page }) => {
		...
		await test.step('Check bottom bar links exist', async () => {
			...
			await expect(page.getByRole('link', { name: 'Changelog', exact: true })).toHaveAttribute(
				'href',
				'https://github.com/aleksilassila/reiverr/releases'
			);
		...
		await test.step('Check User Interface section', async () => {
			...
			await expect(page.getByRole('button', { name: 'Save Changes' })).toHaveClass(
				/cursor-not-allowed/
			);
			...
			await expect(page.getByRole('button', { name: 'Save Changes' })).not.toHaveClass(
				/cursor-not-allowed/
			);
			await page.getByRole('combobox').first().click();
		...
		await test.step('Check Discovery section', async () => {
			...
			await expect(
				page.getByRole('heading', { name: 'Exclude library items from Discovery', exact: true })
			).toBeVisible();
			await expect(page.locator('.w-11').nth(1)).toBeEnabled();
			await expect(
				page.getByText(
					'Filter results based on spoken language. Takes ISO 639-1 language codes separated with commas. Leave empty to disable.'
				)
			).toBeVisible();
