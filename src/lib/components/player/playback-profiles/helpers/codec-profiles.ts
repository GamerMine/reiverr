...
function createProfileCondition(
	Property: JellyfinProfileConditionValue,
...
export function getAacCodecProfileConditions(
	...
	const conditions: JellyfinProfileCondition[] = [];
 ...
 */
export function getCodecProfiles(videoTestElement: HTMLVideoElement): JellyfinCodecProfile[] {
	...
	const hevcCodecProfileConditions: JellyfinProfileCondition[] = [
	...
	const h264CodecProfileConditions: JellyfinProfileCondition[] = [
		createProfileCondition('IsAnamorphic', 'NotEquals', 'true'),
	...
	if (isApple() && isMobile() && Number(safariVersion()) < 13) {
		const codecProfile: JellyfinProfileCondition = {
			Type: 'Video',
			Codec: 'h264',
			Container: 'ts',
