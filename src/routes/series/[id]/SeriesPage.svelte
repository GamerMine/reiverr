<script lang="ts">
    ...
    async function addToSonarr() {
        ...
        addSeriesToSonarr(tmdbId, qualityProfileId)
            .then(() => {
                refreshSonarr();
                notificationStore.createSuccess("Episode(s) added to queue", "The episode(s) will be added to the library once available.")
            });
    }
    ...
    async function openRequestModal() {
        ...
        modalStore.create(SeriesRequestModal, {
                ...
                {:else}
                    ...
                    {:else if !$sonarrSeriesStore.item && settings.globalSettings.sonarr.baseUrl}
                        <Button variant="primary" disabled={addToSonarrLoading} onclick={addToSonarr}>
                            <Plus size="20"/>
                            <span>{$_('library.content.get')}</span>
                        </Button>
                    {:else if $sonarrSeriesStore.item}
                        <Button variant="primary" disabled>
                            <ActivityLog size="20"/>
                            <span class="ml-2">{$_('library.content.inqueue')}</span>
                        </Button>
                    {/if}
                {/if}
            </div>
        ...
        {#snippet servarr_components()}
            ...
            {#if sonarrSeries}
                ...
                <div class="flex gap-4 flex-wrap col-span-4 sm:col-span-6 mt-4">
                    <Button>
                        <span class="mr-2">{$_('library.content.manage')}</span>
                        <Archive size="20"/>
                    </Button>
                </div>
