<script lang="ts">
	...
	import PageDots from '$lib/components/controls/PageDots.svelte';
	...
	import { getJellyfinBackdrop } from '$lib/utils/utils.ts';
	import { onMount } from 'svelte';

	let hideUI = false;

	let continueWatchingEmpty = $state(false);

	let nextUpProps = $derived.by(async () => {
		const nextUpP = jellyfinGetNextUp();
		const continueWatchingP = jellyfinGetContinueWatching();

		const [nextUp, continueWatching] = await Promise.all([nextUpP, continueWatchingP]);
		const items = [
		...
		return await Promise.all(
			items?.map(async (item) => {
				const parentSeries = await jellyfinGetItems().then((items_1) =>
					items_1.data?.find((i_1) => i_1.Id === item.SeriesId)
				);
